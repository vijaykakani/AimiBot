    ...
    public class TriviaGame
    {
        ...
        public async Task EnsureStopped()
        {
            ...
            else
            {
                ...
                {
                    int reward = kvp.Value * NadekoBot.BotConfig.TriviaCurrencyRewardMultiplier;
                    if (reward > 0)
                    {
                        await CurrencyHandler.AddCurrencyAsync(kvp.Key, "Trivia Score Points", reward, false).ConfigureAwait(false);
                        sb.AppendLine($"**{kvp.Key.Username}** has {kvp.Value} points".ToString().SnPl(kvp.Value) + " and earned " + reward + " " + NadekoBot.BotConfig.CurrencySign);
                    }
                    else
                    {
            ...
            await channel.EmbedAsync(new EmbedBuilder().WithOkColor()
                    ...
                    .WithDescription(sb.ToString())).ConfigureAwait(false);
        }
        ...
        private async Task PotentialGuess(SocketMessage imsg)
        {
            try
            {
                ...
                if (Users[guildUser] == WinRequirement)
                {
                    ...
                    /*
                    string debugMsg;
                    debugMsg = $"[DEBUG] Trivia Currency Reward: " + NadekoBot.BotConfig.TriviaCurrencyReward;
                    await umsg.Channel.SendMessageAsync(debugMsg).ConfigureAwait(false);
                    */

                    /*  points giving: old version
                    var reward = TriviaCurrencyReward;
                    if (reward > 0)
                    {
                        await CurrencyHandler.AddCurrencyAsync(guildUser, "Won trivia", reward, false).ConfigureAwait(false);
                        await textChannel.SendConfirmAsync($"**{guildUser}** is awarded {reward}{NadekoBot.BotConfig.CurrencySign}!").ConfigureAwait(false);
                    }
                    */
                    return;
                }
